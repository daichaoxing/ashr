...
#' sebetahat = abs(rnorm(200,0,1))
#' betahat = rnorm(200,beta,sebetahat)
#' beta.ashn = ashn(betahat, sebetahat,mu=6)
#' #beta.ashn4 = ashn(betahat, sebetahat,mu=6,ncores=4)
...
ashn=function(betahat,sebetahat, 
              mixcompdist = c("uniform","halfuniform","normal"),
			  df=NULL, mu=4,ncores=FALSE,
              ...){
  if(length(mu)==1){
    mutemp=seq(from=(mean(betahat)-3*sd(betahat)),to=(mean(betahat)+3*sd(betahat)),length=mu)
